...
> **確定済み（v0.2で閉じた残課題）：**
> - D-01 Levenshtein比較対象 → **title（原文・英語）に確定**（RED-03対応）
> - D-02 C-05試合日程の取得頻度 → **4時間バッチと同周期に確定**（YEL-06対応）
> - D-05 Renderのデプロイ方式 → **GitHubリポジトリ連携（自動デプロイ）に確定**（RED-01対応）
