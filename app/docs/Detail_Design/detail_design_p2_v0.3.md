...
| v0.3 | 2026-06-14 | NEW-YEL-01対応：§2-1 dedup.pyの定数定義（DEDUP_THRESHOLD・DEDUP_WINDOW_DAYS）を削除し`from config import DEDUP_THRESHOLD, DEDUP_WINDOW_DAYS`に置き換え。YEL-01（設定の一元化）をdedup.pyに適用漏れなく反映。これにより全定数をconfig.pyのみで管理する方針が完全適用された。 |
