...
| 5 | JWT認証フローのセキュリティ面（トークン保存先D-06・有効期限）が記述されているか | ⬜ |
| 6 | 残課題D-04（game_schedule保持期間）・D-06（JWTトークン保存先）の扱いが明記されているか | ⬜ |
...
SECRET_KEY = os.getenv("SECRET_KEY", "")   # 必須・未設定時は起動エラー
ALGORITHM  = "HS256"
...
def init_user(db):
    ...
    if not username or not password:
        raise RuntimeError("USERNAME / USER_PASSWORD が未設定です")
...
| エラー種別 | 対応 |
|---|---|
| `USERNAME` / `USER_PASSWORD` が未設定 | 起動時に `RuntimeError` → サービス起動失敗（intentional） |
...
async def get_news(
    ...
    # is_duplicate=True の記事は crud.get_articles() 内でフィルタ済み
    return {
...
export function useAuth() {
    const [token, setToken] = useState(null);  // メモリ上に保持（D-06: 詳細設計で保存先確定）
...
> **D-06（未確定）：** `token` をメモリ（Reactステート）に保持するか、`sessionStorage` / `localStorage` に保存するかは詳細設計で確定する。メモリ保持はページリロードでログアウトになる。実装前に確定が必要。
...
// 2. カテゴリリスト変更（フェーズ1: all/trade/contract/game/column）
const CATEGORIES = ["all", "trade_fa", "draft", "schedule", "injury", "column"];
// ※ "schedule" は /api/schedule から取得（articlesとは別フック）

// 3. ネタバレ防止OFF→ON戻し（F-05b対応）
// revealedIds は revealScore() で追加のみだったが、resetSpoiler() で再ぼかしを可能にする
...
- 適用条件変更：`category === "game"` に加えて全カテゴリの `has_score=true` に適用
...
function GameRow({ game }) {
    ...
    const showScore = !game.has_score || revealed;
...
|---|---|---|---|
| JWT検証失敗（無効・期限切れ） | API Server | HTTP 401 を返す | フロントエンドでログイン画面にリダイレクト |
| ログイン失敗（認証失敗） | POST /api/auth/login | HTTP 401 を返す | LoginForm にエラーメッセージ表示 |
| `SECRET_KEY` 未設定 | 起動時 | 起動失敗（RuntimeError） | — |
| `USERNAME` / `USER_PASSWORD` 未設定 | 起動時 initスクリプト | 起動失敗（RuntimeError） | — |
...
| # | 課題 | 詳細設計での確定内容 |
|---|---|---|
| D-04 | game_schedule の保持期間 | articles と同様30日か、別途定義するか。`delete_old_game_schedule()` 実装方針に直結 |
| D-06 | JWTトークンのフロントエンド保存先 | メモリ（ページリロードでログアウト）/ sessionStorage（タブ閉じでログアウト）/ localStorage（永続）のいずれか。§5-1 の実装に直結 |
