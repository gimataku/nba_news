...
| 確認項目 | 確認内容 | 結果 |
|---|---|---|
| 1-A | HTTPステータスが200か（認証エラー401・有料制限402でないか） | ✅ OK |
| 1-B | `data`配列に試合データが含まれるか | ✅ OK |
| 1-C | `date`フィールドに日時が含まれるか | ✅ OK　|
| 1-D | `home_team.city`または会場情報が含まれるか | ✅ OK　|
| 1-E | オフシーズン期間（2026年6月）でも404にならないか | ? |
...
| 確認項目 | 確認内容 | 結果 |
|---|---|---|
| 2-A | HTTPステータスが200か（401・402・404でないか） | ❌ NG |
| 2-B | `data`配列に順位データが含まれるか | ❌NG |
| 2-C | `wins`・`losses`・`win_pct`フィールドがあるか | ❌ NG |
| 2-D | `games_behind`（GB）フィールドがあるか | ❌ NG |
| 2-E | カンファレンス区別（`conference`フィールド等）があるか | ❌ NG |
