...
⑤ Claude API処理（processor/claude_client.py）
   api_limit_exceeded が "true" の場合はスキップ
   各記事を1リクエストで翻訳・要約・分類
        ...
        for attempt in range(source["retry"]):
            try:
                feed = feedparser.parse(
                    source["url"],
                    request_headers={"User-Agent": "NBANewsJP/1.0"}
                )
                if feed.entries:
                    return feed.entries, source["name"], (i > 0)
            except Exception:
                time.sleep(1)
    return [], "", True
```
...
CREATE TABLE articles (
    ...
    link             TEXT    NOT NULL UNIQUE,   -- 重複排除キー
    title_original   TEXT    NOT NULL,           -- 英語見出し（原文）
    title_ja         TEXT,                       -- 翻訳済み見出し（API上限時はNULL）
    summary_ja       TEXT,                       -- 日本語要約（API上限時はNULL）
    category         TEXT,                       -- trade / contract / game / column
    ...
    source_url       TEXT    NOT NULL,           -- 元記事URL（= link と同値）
...
export function useNews() {
    ...
    return {
        ...
        setSelectedCategory, setSpursOnly,
