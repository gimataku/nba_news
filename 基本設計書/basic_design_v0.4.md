...
┌──────────────────────────────────────────────────────────────┐
│                        外部サービス                           │
│  Hoops Rumors RSS   Hoops Wire RSS      BALLDONTLIE API          │
│  （ニュース主軸）    （フェールオーバー1）  （試合スコア）           │
│              The Cold Wire NBA RSS                                │
│              （フェールオーバー2）                                 │
│                           Claude API                          │
│                      （翻訳・要約・分類）                      │
└───────────────────────────┬──────────────────────────────────┘
...
| RSS Fetcher | Hoops Rumors（主軸）/ Hoops Wire（フェールオーバー1）/ The Cold Wire NBA（フェールオーバー2）からRSSを取得 | Python（feedparser） |
...
[Scheduler]
    ...
    ▼
[月次リセットチェック]
    │ 現在月（YYYY-MM）が app_settings.api_reset_month と異なる場合:
    │   → app_settings.api_limit_exceeded = "false" にリセット
    │   → app_settings.api_reset_month = 現在月（例: "2026-05"）に更新
    ▼
[RSS Fetcher]
    │ Hoops Rumors RSS取得
    │   └─ 失敗（タイムアウト/HTTP 4xx/5xx）
    │         └─ Hoops Wire RSS取得（フェールオーバー1）
    │               └─ 失敗
    │                     └─ The Cold Wire NBA RSS取得（フェールオーバー2）
...
[重複チェック（DB）]
    ...
    ▼
[Filter: Spursチェック]
    │ 優先順位1: <category>タグに「Spurs」「San Antonio」等が含まれるか確認
    │ 優先順位2: <category>タグが存在しない または マッチなし
    │             → <title> と <description> にキーワードマッチを試みる
    │             （Hoops Wire・The Cold Wire NBA等のフェールオーバーソースに対応）
    │ 優先順位3: いずれにもマッチなし → is_spurs = False
...
[Score Fetcher: BALLDONTLIE]
    ...
    │
    │ 1. 記事のpubDateから対象日を決定
    │      pubDateが当日 00:00〜11:59 → 前日の試合を検索
    │      pubDateが当日 12:00〜23:59 → 当日の試合を検索
    │
    │ 2. BALLDONTLIE /v1/games?team_ids[]=27
    │      &start_date=YYYY-MM-DD&end_date=YYYY-MM-DD でSpurs試合を検索
    │      （SpursのチームID=27）
    │
    │ 3. 試合が見つかった場合
    │      → game_id・最終スコア・クォーター別スコア・勝敗を score_data（JSON）に保存
    │
    │ 4. 試合が見つからなかった場合
    │      → score_data = NULL
    │      → フロントエンドは「スコアを表示」ボタンを非表示にする
    ▼
[DB保存: articles テーブル]
    │ 記事データ一式を保存
    ▼
...
### 4-1-2. ニュースソース一覧（実機確認済み・確定版）

| 優先順位 | ソース | RSS URL | categoryタグ構成 | Spursフィルタ適性 |
|---|---|---|---|---|
| 主軸 | **Hoops Rumors** | `hoopsrumors.com/feed` | チーム名・選手名（実機確認済み） | ✅ キーワードマッチ可 |
| フェールオーバー1 | **Hoops Wire** | `hoopswire.com/feed` | チーム名のみ（例：`Spurs`）（実機確認済み） | ✅ 「Spurs」直接記載で最適 |
| フェールオーバー2 | **The Cold Wire NBA** | `thecoldwire.com/sports/nba/feed` | `{チーム名} Rumors And News (Updated Daily)`形式（実機確認済み） | ✅ `San Antonio Spurs`キーワードマッチ可 |
| 除外 | ~~HoopsHype~~ | — | RSS 404（実機確認済み） | ❌ |
| 除外 | ~~RealGM~~ | — | RSS 404（実機確認済み） | ❌ |
...
Hoops Rumors取得試行（最大2回リトライ・1秒インターバル）
    └─ 失敗
        ↓
Hoops Wire取得試行（最大2回リトライ）　← フェールオーバー1
    └─ 失敗
        ↓
The Cold Wire NBA取得試行（最大2回リトライ）　← フェールオーバー2
    └─ 失敗
        ↓
fetch_logsにエラー記録
...
| カラム | 内容 |
|---|---|
| source_used | 実際に使用したRSSソース名（hoops_rumors / hoops_wire / the_cold_wire） |
...
| source | TEXT | ソース名（hoops_rumors / hoops_wire / the_cold_wire） |
...
| key | 初期値 | 説明 |
|---|---|---|
| `spoiler_guard_enabled` | `"true"` | ネタバレ防止ON/OFF |
| `spurs_filter_enabled` | `"false"` | Spursフィルタ（デフォルト全チーム表示） |
| `last_fetched_at` | `""` | 前回バッチ実行日時（ISO8601）。`""`の場合（初回起動時）は即時バッチ実行 |
| `api_limit_exceeded` | `"false"` | Claude API上限超過フラグ |
| `api_reset_month` | `""` | 最終リセット月（例："2026-05"）。現在月と異なる場合にapi_limit_exceededをリセット |
...
| メソッド | パス | 内容 |
|---|---|---|
| GET | `/api/articles` | ニュース記事一覧取得（score_dataを含む） |
| GET | `/api/settings` | アプリ設定取得 |
| PUT | `/api/settings` | アプリ設定更新 |
| GET | `/api/status` | 最終取得日時・フェールオーバー状況取得 |
| POST | `/api/fetch` | 手動バッチ実行トリガー |

> **設計方針（方針A採用）：** スコアデータ（score_data）は `GET /api/articles` のレスポンスに常時含める。ローカル限定アプリのためネタバレ情報の分離は不要と判断。フロントエンドはボタン押下時にレスポンス済みの score_data をローカルで表示するのみで、追加APIリクエストは発生しない。
...
【月次リセット（致命-2対応）】
バッチ処理起動時（Scheduler）に以下を実行:
→ 現在月（YYYY-MM）と app_settings.api_reset_month を比較
→ 異なる場合:
    app_settings.api_limit_exceeded = "false" にリセット
    app_settings.api_reset_month = 現在月 に更新
```
...
| T-02 | フェールオーバー | Hoops Rumors障害時にHoops Wire→The Cold Wire NBAの順で切り替わるか |
...
| T-15 | フェールオーバー時Spursフィルタ | Hoops Wire・The Cold Wire NBA使用時にSpursフィルタ（categoryタグおよびtitle/descriptionフォールバック）が正しく機能するか |

> **注記（軽微-1）：** `requirements_v0.6` のセクション5「主要リスクサマリー」の参照先が `risk_report_v0.4.md` と古い表記になっている。次バージョン（`requirements_v0.7`）更新時に `risk_report_v0.5.md` に修正すること。
...
| No. | 事項 | 状況 | 結果 |
|---|---|---|---|
| 1 | **Hoops WireのRSS実機確認** | ✅ 完了 | `<category>`タグに「Spurs」が直接記載されることを確認。フェールオーバー1ソースとして採用確定 |
| 2 | **The Cold Wire NBAのRSS実機確認** | ✅ 完了 | `<category>`タグに`San Antonio Spurs Rumors And News (Updated Daily)`形式でチーム名が含まれることを確認。フェールオーバー2ソースとして採用確定 |
| 3 | ~~HoopsHypeのRSS実機確認~~ | ❌ 除外確定 | RSS 404のため除外。Hoops Wire・The Cold Wire NBAに代替 |

**全必須事項が完了。詳細設計（手順4）に着手可能。**
...
| v0.2 | 2026-05-10 | レビュー指摘（致命2件・改善3件・軽微3件）を全件反映。①Score FetcherにpubDate→対象日決定→BALLDONTLIE照合→NULL時の画面挙動を追記（致命-1）、②Schedulerに月次リセットロジックを追加・app_settingsにapi_reset_monthカラムを追加（致命-2）、③Spursフィルタにcategoryタグ→title/descriptionの2段階フォールバックを追記・HoopsHype実機確認を詳細設計前必須事項に追加（改善-1）、④スコアエンドポイントを廃止し方針A（GET /api/articlesにscore_data含める）を採用・明記（改善-2）、⑤T-15（フェールオーバー時Spursフィルタ）をテスト観点に追加（改善-3）、⑥requirements_v0.7更新時の参照先修正注記を追加（軽微-1）、⑦last_fetched_at=""時の即時実行をapp_settings説明に明記（軽微-2）、⑧.env.example定義内容を§9に追記（軽微-3） |
| v0.3 | 2026-05-10 | Hoops Wire・The Cold Wire NBAのRSS実機確認結果を反映。①HoopsHype（RSS 404）を除外、②Hoops Wire（categoryタグ=「Spurs」直接記載）をフェールオーバー1に採用、③The Cold Wire NBA（categoryタグ=「San Antonio Spurs Rumors And News」形式）をフェールオーバー2に採用、④§1-1構成図・§1-2コンポーネント一覧・§4-1ソース一覧テーブル・§4-2フェールオーバー動作・§4-3source_used定義を全件更新、⑤§12必須事項を全完了に更新 |
| v0.4 | 2026-05-10 | レビュー指摘（要改善2件・軽微2件）を全件反映。①§3-1のRSS Fetcherフェールオーバー記述をHoops Wire→The Cold Wire NBAの2段階に修正（要改善-1）、②§5-2 articlesテーブルのsourceカラム定義値を`hoops_wire / the_cold_wire`に修正（要改善-2）、③T-02のフェールオーバー先をHoops Wire→The Cold Wire NBA順に修正（軽微-1）、④T-15のフェールオーバー先をHoops Wire・The Cold Wire NBAに修正（軽微-2） |
