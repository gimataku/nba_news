...
| 項目 | 内容 |
|---|---|
| **テスト対象** | `processor/claude_client.py` – `process_article()` プロンプト精度 |
| **目的** | 4分類の境界条件が要件定義書の定義通りに判定されること |
| **テストケース** |（各ケースでモックの代わりに実際のClaude APIを呼び出す統合確認） |

| ケース | 入力記事の内容 | 期待カテゴリ |
|---|---|---|
| TC-06-1 | トレード交渉報道（スコアなし） | `trade` |
| TC-06-2 | FA契約金額の報道 | `contract` |
| TC-06-3 | 試合速報（スコアが主要情報） | `game` |
| TC-06-4 | 試合分析コラム（スコアが引用として含まれる） | `column` |
| TC-06-5 | トレード成立後の選手コメント | `trade` |
